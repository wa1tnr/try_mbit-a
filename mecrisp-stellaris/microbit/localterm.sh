...
if [ $# -lt 1 ]; then
    ...
    echo "Use:  ./localterm.sh  /dev/ttyACM0"
    exit 1
fi
...
# micro:bit adjusted timing for ascii-upload March 2022:
picocom -e \\ -f n -p n -d 8 -b 115200 --omap delbs,crlf --imap lfcrlf --send-cmd "ascii-xfr -sn -l 50 -c 8" ${1}
